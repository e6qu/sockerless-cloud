...
func registerWebStaticSites(srv *sim.Server) {
	staticID := func(r *http.Request) string {
	...
	// GET staticSites (list by subscription).
	srv.HandleFunc("GET /subscriptions/{subscriptionId}/providers/Microsoft.Web/staticSites", func(w http.ResponseWriter, r *http.Request) {
		prefix := "/subscriptions/" + sim.PathParam(r, "subscriptionId") + "/"
		out := webStaticSites.Filter(func(s StaticSiteResource) bool { return strings.HasPrefix(s.ID, prefix) })
		sim.WriteJSON(w, http.StatusOK, map[string]any{"value": out})
	})

	// GET staticSites (list by resource group).
	srv.HandleFunc("GET /subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Web/staticSites", func(w http.ResponseWriter, r *http.Request) {
	...
	// GET staticSites/{name}
	srv.HandleFunc("GET /subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Web/staticSites/{name}", func(w http.ResponseWriter, r *http.Request) {
		s, ok := webStaticSites.Get(staticID(r))
		if !ok {
			sim.AzureErrorf(w, "ResourceNotFound", http.StatusNotFound,
	...
	// PUT staticSites/{name} — create or update (a real Azure LRO; the sim
	// completes synchronously with the final resource).
	srv.HandleFunc("PUT /subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Web/staticSites/{name}", func(w http.ResponseWriter, r *http.Request) {
		...
		sim.WriteJSON(w, http.StatusOK, ss)
	})

	// PATCH staticSites/{name}
	srv.HandleFunc("PATCH /subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Web/staticSites/{name}", func(w http.ResponseWriter, r *http.Request) {
		ss, ok := webStaticSites.Get(staticID(r))
		if !ok {
			sim.AzureErrorf(w, "ResourceNotFound", http.StatusNotFound,
				"The Resource 'Microsoft.Web/staticSites/%s' was not found.", sim.PathParam(r, "name"))
			return
		...
		if req.Properties != nil {
			if req.Properties.RepositoryURL != "" {
				ss.Properties.RepositoryURL = req.Properties.RepositoryURL
			}
			if req.Properties.Branch != "" {
				ss.Properties.Branch = req.Properties.Branch
			}
		}
		webStaticSites.Put(ss.ID, ss)
		sim.WriteJSON(w, http.StatusOK, ss)
	...
	// DELETE staticSites/{name}
	srv.HandleFunc("DELETE /subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.Web/staticSites/{name}", func(w http.ResponseWriter, r *http.Request) {
		if webStaticSites.Delete(staticID(r)) {
			w.WriteHeader(http.StatusOK)
			return
		}
		w.WriteHeader(http.StatusNoContent)
