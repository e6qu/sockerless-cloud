...
| `azure-compute_vm_patches` | direct | not applicable | not applicable | `simulator-azure/sdk-tests/compute_vm_patches_test.go`; `az vm` exposes no assess/install-patches or capture command, and `assessPatches`, `installPatches` and `capture` are imperative verbs the `azurerm` provider (v5.2.0) wraps no resource for |
