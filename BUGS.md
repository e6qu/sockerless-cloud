...
- **BUG-3010 (CloudWatch Logs reports every log group as storing nothing):**
  `CWLogGroup.StoredBytes` (`simulator-aws/cloudwatch.go`) is never written,
  so `DescribeLogGroups` answers `storedBytes: 0` for every group — the four
  ecs-dev-desktop groups on the Scaleway stack did on 2026-09-17 while
  holding weeks of events. CloudWatch Logs documents the member only as "the
  number of bytes stored", without saying whether it counts the compressed
  archive or the ingested bytes, so a figure computed here would be a guess.
  Fix shape: settle what the service counts against a real account, then keep
  the figure per group as events are put and expired.
