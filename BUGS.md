...
|----|-----|------|---------|-----------|
| 2952 | P3 | Azure virtual-machine extension, operation and patch surfaces have no Terraform coverage | a surface reachable through a provider resource is exercised by two of the three clients, and nothing said so | The three surfaces are served and covered by the SDK and the CLI through `compute_network_request_validation_test.go`, but `simulator-azure/terraform-tests/main.tf` declares no `azurerm_virtual_machine_extension`, run-command or patch-assessment resource, so the Terraform client never reaches them. The provider does expose them, so this is a gap rather than a surface Terraform cannot express — which is why the coverage matrix records it as tracked against this entry rather than "not applicable". It became visible only when the surface tables began resolving routes composed from a version prefix: all three surfaces had no table at all before that, so no cell existed to be empty. Fix shape: add the extension, run-command and patch resources to the Azure Terraform stack, then flip the three matrix cells to direct. |
