...
- ~~**BUG-3038 (nine services' error writers spelled the message member their
  ...
  turned `TestGlue_DataQualityLifecycle_SDK` red: its error message arrived
  empty. The SDK's schema-driven deserializers read an exception's message only
  under the member its model declares, and Glue's writer wrote `message` where
  all 49 Glue exceptions declare `Message`; CloudTrail's and Budgets' writers
  had the same mismatch for every error, and ACM's and WAFv2's for some. The
  nine service writers now take the spelling from a table generated from the
  vendored models — the first half of BUG-3030's repair.
