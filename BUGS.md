...
|----|-----|------|---------|-----------|
| 3016 | P2 | Seven routes of the Amazon S3 control plane pass through no IAM enforcement | the operation's IAM action belongs to a service namespace this repository vendors no Service Reference for, so neither the action nor its resource types are derivable here | Every other `/v20180820` route is authorized as the action `specs/cloud-api/aws/service-reference/s3.servicereference.json.gz` names for its operation, against the resource the request names. Seven are not, because that reference declares no action for their operations at all: `PutAccessPointScope`, `GetAccessPointScope`, `DeleteAccessPointScope` and `ListAccessPointsForDirectoryBuckets` are `s3express:` actions, the control plane's `ListRegionalBuckets` and `DeleteBucketLifecycleConfiguration` are `s3-outposts:` ones, and `WriteGetObjectResponse` is `s3-object-lambda:WriteGetObjectResponse`. Guessing either half denies a grant real AWS honours, so each route is listed in `s3ControlUngatedRoutes` with its namespace and `TestEveryS3ControlRouteIsGatedOrDeclaredUngated` fails on an eighth. Fix shape: `scripts/fetch-aws-service-reference.sh s3express s3-outposts s3-object-lambda`, then gate the seven from what those documents declare and delete their entries; the resource ARN formats come with the reference, so nothing else has to be decided by hand. |
