...
|----|-----|------|---------|-----------|
| 2951 | P2 | Amazon ECS service reports a deployment rollout COMPLETED while no task of it runs | a state machine reports a terminal outcome its own counts contradict, so a wait on either one can hang | `TestAmazonECSServiceDeploymentFailureStateSurvivesSimulatorRestart_SDK` timed out on 2026-08-29 in `sim (aws sdk services-a-m)` (run 33218978079), and the wait's own diagnostic reports the service on `persistent-deployment-task:2` with `running 0 of 1 desired, 1 pending, rollout COMPLETED`. A rollout is not COMPLETED while a task is pending and none is running — those three cannot all hold, so the rollout state is set from something other than the task counts. The test drives the deployment circuit breaker with `ThresholdType=COUNT, Value=2`: it waits for the first launch failure, hard-replaces the simulator, and expects the second failure to reach the threshold and roll back to `:1`. Only one `was unable to place a task` event was recorded and the retry stayed pending, so the second failure never landed inside the 30s wait. Two candidates, and the inconsistent rollout state is why the first is the suspect: the post-restart reconciler does not resume the persisted retry deadline (re-launching instead, resetting the clock), or it does and the container run exceeds the window on a loaded runner. Not caused by the branch it surfaced on — nothing there touches ECS deployment, persistence or reconciliation, the same shard passed on that branch's previous push with the same re-vendored ECS model, and neither recent `main` failure is this test. Fix shape: derive `rolloutState` from the deployment's own running/pending/desired counts so it cannot read COMPLETED with none running, then assert the persisted retry deadline is honoured across a restart rather than restarted. |
