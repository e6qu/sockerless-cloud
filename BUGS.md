...
|----|-----|------|---------|-----------|
| 2953 | P2 | An Amazon-style capture of an Azure virtual machine is unreachable, because the machine's disk does not outlive its guest process | two preconditions that are each right on their own describe, together, a call sequence that cannot exist | `VirtualMachines_Capture` requires the machine to have been generalized, and `VirtualMachines_Generalize` refuses a machine that is running — both faithful to Azure. But `azureCaptureVMImage` reads `rootfs.ext4` out of the live guest's working directory through `azureGuestFor`, which errors for a machine that is not running, and `azureStopRealVM` deletes the guest record and its working directory when the machine stops. So generalizing first destroys the disk the capture needs, and capturing first is refused for want of generalization: no order of calls reaches the operation. In Azure the managed disk outlives the machine, which is what makes deallocate → generalize → capture the normal sequence, and it is also why a deallocated machine restarts with its data. Found by `TestCompute_VirtualMachinePatchOperations` on Linux CI, where generalize answered 409 `OperationNotAllowed` for the running machine; the test now asserts that refusal and leaves the capture to this entry. Fix shape: give the machine's disk a lifetime of its own — a path derived from the virtual machine's id rather than the guest's temporary working directory — so it survives a stop, then have the capture read it without requiring a live guest and quiesce only when one is running. |
