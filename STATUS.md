...
- **Request-path indexes**: every handler wrapper that decides whether to claim
  ...
  guard — to a floor that may only fall, now 27. Parent-scoped child
  ...
  AWS Amplify hosted-content path, and the Route 53 CNAME search that both
  certificate validation and domain verification make. Each conversion is held
