...
- **A pin must carry the working tree's content**, and `scripts/check-support-module-pins.sh` fails when it does not. A change to a support module therefore lands in two pushes: push the branch, then pin the pushed commit in every requiring module (`go get github.com/e6qu/sockerless-cloud/<module>@<sha>`; Go records a pseudo-version) and push again. A squash merge is content-identical to the branch head it squashed, so the pin stays current on `main`. `scripts/check-installable-build.sh` proves the pinned graph compiles with `GOWORK=off`; it cannot see a pin that compiles but lags.
